...
public class SingleSelectSongUI : MonoBehaviour
{
    ...
    List<EZR.SongList.SongInfo> currentList = new List<EZR.SongList.SongInfo>();
    ...
    int currentSongIndex = EZR.SongList.CurrentIndex;
    ...
    void filterSongs()
    {
        ...
        foreach (var info in EZR.SongList.List)
        {
            ...
            if (info.difficult.ContainsKey(currentMode))
                isHit = true;
            else if (info.difficult.ContainsKey(EZR.SongList.DJMaxModeMapping(currentMode)))
                isHit = true;
    ...
    // 歌曲难度状态
    bool[] songDifficultState(EZR.SongList.SongInfo info)
    {
        bool ez = false;
        bool nm = false;
        bool hd = false;
    ...
    void updateList()
    {
        ...
        songsListScrollRect.verticalNormalizedPosition =
            1 - Mathf.Clamp01(
                    ((currentList.IndexOf(EZR.SongList.List[currentSongIndex]) - songUICount / 2 + 1) * songUIHeight) / (songsListContent.sizeDelta.y - songListHeight));
    ...
    Transform updateSongUI(bool isRefresh)
    {
        ...
        for (int i = 1; i < songsListContent.childCount; i++)
        {
            ...
            if (isRefresh || isLoop)
            {
                ...
                songUIComp.SongIndex = EZR.SongList.List.IndexOf(songInfo);
    ...
    {
        EZR.SongList.CurrentSortMode = EZR.SongList.SortMode.ByName;
        var btn = EventSystem.current.currentSelectedGameObject.GetComponent<EZR.ButtonExtension>();
        if (btn.IsSelected)
        {
            EZR.SongList.IsAscending = true;
            btn.SetSelected(false);
        }
        else
        {
            EZR.SongList.IsAscending = false;
    ...
    {
        EZR.SongList.CurrentSortMode = EZR.SongList.SortMode.ByDifficult;
        var btn = EventSystem.current.currentSelectedGameObject.GetComponent<EZR.ButtonExtension>();
        if (btn.IsSelected)
        {
            EZR.SongList.IsAscending = true;
    ...
    void updateGameType()
    {
        ...
        foreach (Transform btn in gameModeContent)
        {
            ...
            if (typeUI.GameType == currentType)
            {
                ...
                EZR.SongList.DJMaxModeMapping(typeUI.GameMode) == currentMode)
                {
    ...
    {
        var panelDifficult = transform.Find("PanelDifficult");
        var state = songDifficultState(EZR.SongList.List[currentSongIndex]);
    ...
    async void setCurrentSong(Transform songUI, string state)
    {
        ...
        var info = EZR.SongList.List[currentSongIndex];
    ...
    public async void BtnStart()
    {
        ...
        var mode = EZR.SongList.List[currentSongIndex].GetCurrentMode(currentMode, currentDifficult);
        ...
        EZR.SongList.CurrentIndex = currentSongIndex;
